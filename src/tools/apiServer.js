...
server.post("/comments/", function(req, res, next) {
  ...
  if (error) {
    ...
    req.body.comment = createComment(req.body.title); // Generate a comment for new comments.
    next();
  }
});
...
// Returns a URL friendly comment
...
function validateComment(comment) {
  if (!comment.title) return "Title is required.";
  if (!comment.category) return "Category is required.";
  if (!comment.userComment) return "Comment is required.";
